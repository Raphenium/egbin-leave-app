$(function() {
	...
	var $table = $('#restaurantsListTable');
	...
	if ($table.length) {
		...
		if (window.categoryId == '') {
			jsonUrl = window.contextRoot + '/json/data/all/restaurants';
		} else {
		...
		$table.DataTable({
					...
					lengthMenu : [ [ 1, 5, 10, -1 ],
							[ '1 Records', '5 Records', '10 Records', 'ALL' ] ],
					pageLength : 1,
					ajax : {
						url : jsonUrl,
						dataSrc : ''
	...
	// code for jquery dataTable
	var $tabless = $('#staffTable');

	// execute the below code only where we have this table
	if ($tabless.length) {
		// console.log('Inside the table!');

		var jsonUrl = window.contextRoot + '/json/data/admin/all/staff';
		...
		$tabless.DataTable({
					...
					columns : [
							...
							{
								data : 'annualLeaveBallance'
							},
							{
								data : 'sickLeaveBallance'
							},
							{
								data : 'examLeaveBallance'
							},
							{
									data : 'id',
									bSortable : false,
...
var $tableu = $('#leaveHistory');

	// execute the below code only where we have this table
	if ($tableu.length) {
		// console.log('Inside the table!');

		var jsonUrl = window.contextRoot + '/json/data/admin/all/leave';
							...
							data : 'status',
								mRender : function(data, type, row) {
									...
									if (data == "PENDING") {
										...
										return '<a href="" class="btn btn-primary">Approve</a> &#160; <a href="" class="btn btn-danger">Decline</a> &#160;';
